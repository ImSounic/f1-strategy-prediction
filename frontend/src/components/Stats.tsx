...
export function Stats() {
  ...
  return (
    <section className="border-y border-f1-border bg-f1-card/50">
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {stats.map((stat, i) => (
            <div key={i} className="text-center">
              <div className="font-display font-black text-2xl md:text-3xl text-f1-light">
                {stat.value}
              </div>
              <div className="font-body text-sm text-f1-muted mt-1">{stat.label}</div>
              <div className="font-mono text-xs text-f1-muted/50 mt-0.5">{stat.sub}</div>
            </div>
          ))}
        </div>
      </div>
